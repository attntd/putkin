...
QtObject {
    ...
    function request(id: int, action: string): bool {
        ...
        calls = calls.concat([action === "suspend" || action === "lock" ? "lock-requested" : action]);
        if (automatic) Qt.callLater(() => {
            if (!operation || operation.id !== id) return;
            if (action === "lock" || action === "suspend") confirmLock();
    ...
    function confirmLock(): void {
        ...
        if (operation.action === "suspend") calls = calls.concat(["suspend"]);
