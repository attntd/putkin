#!/usr/bin/env python3
"""Local square compositor borders and theme synchronization: immutable release and guarded restoration of configuration files."""
...
def complete_switch():
    ...
    panels = {}
    for target, method in [('ui', 'openAudio'), ('ui', 'openNotifications'), ('launcher', 'openCommands'), ('launcher', 'openClipboard'), ('ui', 'openQuickSettings')]:
    ...
    assert run(['chezmoi', 'status']) == '', 'Chezmoi differs from live configuration'
    assert all(digest(path) == record['after'] for path, record in PLAN['changes'].items()), 'Configuration changed during activation'
    palette = PLAN['window_appearance']
    expected_first = '0xFF' + palette['primary'][1:].upper()
    expected_last = '0xFF' + palette['secondary'][1:].upper()
    verify_lua = ('function() local g=hl.get_config("general.col.active_border"); '
                  'assert(#g.colors==10 and g.colors[1]=="' + expected_first + '" and g.colors[10]=="' + expected_last + '"); '
                  'assert(math.abs(g.angle-30)<0.001); '
                  'assert(hl.get_config("decoration.rounding")==0 and hl.get_config("general.border_size")==2); '
                  'assert(hl.get_config("general.col.inactive_border").colors[1]=="0xFF45475A") end')
    assert run(['hyprctl', 'dispatch', verify_lua]) == 'ok'
    result = {'window_appearance': palette, 'chezmoi_status': 'clean', 'bar_focus': bar_monitor, 'keyboard': keyboard, 'settings_window': settings_client, 'panel_ipc': panels, 'audio_toggle': True, 'audio_before': before_audio, 'audio_after': after_audio,
