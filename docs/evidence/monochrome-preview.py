...
ShellRoot {
    ...
    MockTray {
        ...
        Component.onCompleted: {
            ...
            items.values[0].icon = Quickshell.iconPath("signal-desktop", true);
        }
    }
...
''')
    ...
    (ROOT/'docs/evidence/monochrome-preview.log').write_text(output)
    print(output)
    assert not qml_errors(output, allow_offscreen_masks=True)
    assert 'PUTKIN_SCREENSHOT_SAVED' in output
    (ROOT/'docs/evidence/monochrome-preview.png').write_bytes((root/'proof.png').read_bytes())
