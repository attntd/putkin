...
STAGE = Path('/tmp/putkin-launcher-icons-update')
EVIDENCE = ROOT / 'docs/evidence'
OLD = json.loads((EVIDENCE / 'caffeinate-focus-activation-plan.json').read_text())
...
assert set(manifest) == set(OLD['manifest'])
changed = [name for name in manifest if manifest[name] != OLD['manifest'][name]]
assert set(changed) == {'shell.qml', 'modules/launcher/LauncherView.qml', 'services/LauncherBackend.qml', 'services/LauncherService.qml'}, changed
...
release = Path('/home/attntd/.local/share/putkin/releases') / ('20260920-launcher-icons-' + release_hash)
...
for target in [STAGE / 'plan.json', EVIDENCE / 'launcher-icons-activation-plan.json']:
    target.write_text(json.dumps(plan, indent=2) + '\n')
activate = (EVIDENCE / 'caffeinate-focus-activate.py').read_text()
activate = activate.replace('Local Caffeinate keyboard focus fix:', 'Local launcher application icons:')
for target in [STAGE / 'activate.py', EVIDENCE / 'launcher-icons-activate.py']:
