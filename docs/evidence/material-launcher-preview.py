...
with tempfile.TemporaryDirectory(prefix='pk-material-') as directory:
    ...
    env.update(HOME=directory, PUTKIN_SCENARIO='launcher', PUTKIN_PREVIEW_WIDTH='1000', PUTKIN_PREVIEW_HEIGHT='700', PUTKIN_SCREENSHOT=str(root/'proof.png'))
...
ShellRoot {
    ...
    PanelPreviewWindow { launcher: launcher; tray: tray }
...
''')
    ...
    (ROOT/'docs/evidence/material-launcher-preview.log').write_text(output)
    print(output)
    assert not qml_errors(output, allow_offscreen_masks=True)
    assert 'PUTKIN_SCREENSHOT_SAVED' in output
    (ROOT/'docs/evidence/material-launcher-preview.png').write_bytes((root/'proof.png').read_bytes())
