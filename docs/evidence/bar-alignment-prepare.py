"""Prepare centered bar content and constant-size charging batteries; preserve desktop changes."""
...
STAGE = Path('/tmp/putkin-bar-alignment-update')
EVIDENCE = ROOT / 'docs/evidence'
OLD = json.loads((EVIDENCE / 'bar-icons-activation-plan.json').read_text())
...
added = {f'assets/material/putkin_battery_android_{level}_charging.svg' for level in [0, 1, 2, 3, 4, 5, 6, 'full']}
modified = {'assets/material/Paths.js', 'assets/material/README.md', 'components/Glyph.qml',
            'core/Metrics.qml', 'core/Icons.js', 'modules/bar/Clock.qml',
            'modules/bar/BarView.qml', 'modules/tray/TrayButton.qml'}
assert set(manifest) - set(OLD['manifest']) == added, 'Unexpected added files'
assert not set(OLD['manifest']) - set(manifest), 'Runtime files removed'
assert set(changed) == added | modified, changed
assert len(manifest) == 268, len(manifest)
release_hash = hashlib.sha256(json.dumps(manifest, sort_keys=True).encode()).hexdigest()[:12]
release = Path('/home/attntd/.local/share/putkin/releases') / ('20260920-bar-alignment-' + release_hash)
...
plan = {
    ...
    'manifest': manifest, 'runtime_files': len(manifest), 'changed_runtime': changed, 'deleted_runtime': [], 'changes': changes,
...
for target in [STAGE / 'plan.json', EVIDENCE / 'bar-alignment-activation-plan.json']:
