"""Prepare the smaller, clock-colored bar icons release and preserve existing desktop changes."""
...
OLD = json.loads((EVIDENCE / 'material-icons-activation-plan.json').read_text())
...
assert set(manifest) == set(OLD['manifest']), 'Runtime file set changed'
assert set(changed) == {'core/Metrics.qml', 'modules/bar/BarView.qml', 'modules/bar/BatteryButton.qml',
                        'modules/bar/WorkspaceStrip.qml', 'modules/tray/TrayButton.qml', 'modules/tray/TrayStrip.qml'}, changed
release_hash = hashlib.sha256(json.dumps(manifest, sort_keys=True).encode()).hexdigest()[:12]
release = Path('/home/attntd/.local/share/putkin/releases') / ('20260920-bar-icons-' + release_hash)
...
for target in [STAGE / 'plan.json', EVIDENCE / 'bar-icons-activation-plan.json']:
