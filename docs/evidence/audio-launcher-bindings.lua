...
dofile("/tmp/putkin-audio-launcher-update/config-2").setup(bindings)
local shell_path="/home/attntd/.local/share/putkin/releases/20260917-audio-launcher-2cf1bad7b39c/shell.qml"
local menu=dofile("/tmp/putkin-audio-launcher-update/runtime/config/menu-keybinds.lua")
menu(bindings.bind,shell_path)
for _,spec in ipairs({{"SUPER + SPACE","launcher toggle"},{"SUPER + V","launcher openClipboard"},{"SUPER + SHIFT + semicolon","launcher openCommands"}}) do
...
dofile("/tmp/putkin-audio-launcher-update/config-1")
