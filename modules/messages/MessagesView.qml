...
FocusScope {
    ...
    function focusInitial(): void {
        if (detail && adapter && adapter.selectedRoute) composer.editor.forceActiveFocus(Qt.TabFocusReason);
        else search.forceActiveFocus(Qt.TabFocusReason);
    }
    function showList(): void { creating = false; detail = false; Qt.callLater(() => list.forceActiveFocus(Qt.TabFocusReason)); }
    Rectangle {
        anchors.fill: parent
    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            UI.TextField {
                ...
                Keys.onReturnPressed: { if (root.filtered.length) root.hub.openConversation(root.filtered[list.currentIndex < 0 ? 0 : list.currentIndex].route); }
            }
            ListView {
                id: list
                ...
                Keys.onPressed: event => {
                    ...
                    else if ((event.key === Qt.Key_L || event.key === Qt.Key_Return || event.key === Qt.Key_Enter) && currentIndex >= 0 && currentIndex < count) root.hub.openConversation(root.filtered[currentIndex].route);
                    else return;
                ...
                delegate: UI.Button {
                    ...
                    onClicked: { list.currentIndex = index; root.hub.openConversation(modelData.route); }
                    contentItem: Column {
                        spacing: Metrics.space4
                        Text {
                            width: parent.width
                    ...
                    UI.FocusIndicator { control: entry; visible: list.activeFocus && list.currentIndex === entry.index }
                }
            }
            UI.NavigationButton {
        ...
        ColumnLayout {
            ...
            MessageHistory {
                ...
                onComposeRequested: composer.editor.forceActiveFocus(Qt.TabFocusReason)
            }
            Text {
                Layout.fillWidth: true
    ...
    Loader {
        ...
        sourceComponent: ConversationDetails { adapter: root.adapter; onDismissed: { root.detailsOpen = false; root.focusInitial(); } }
    }
    Loader {
        id: mediaPreview
        anchors.fill: parent
    ...
    Keys.onEscapePressed: { if (detailsOpen) detailsOpen = false; else if (creating || (narrow && detail)) showList(); else dismissed(); }
