...
ShellRoot {
    ...
    FloatingWindow {
        id: cover
        title: "Syntetyczne inne okno S07"
    ...
    IpcHandler {
        ...
        function stage(paths: string): bool { return adapter.attachFiles(JSON.parse(paths).paths); }
        function remove(aid: string): void { adapter.removeAttachment(aid); }
        function preview(index: int): void {
            const attachments = JSON.parse(adapter.messages.get(index).attachmentsJson);
            controller.window.view.previewAttachment = attachments[0];
        }
        function closePreview(): void { controller.window.view.previewAttachment = null; }
